...
app.put('/room/:roomid', async (req, res) => {
  ...
  if (!roomtype || !price || !status) {
    return res.status(400).json({ message: 'Thiếu thông tin để chỉnh sửa phòng' });
  }
  ...
  try {
    ...
    if (result.affectedRows === 0) {
      return res.status(404).json({ message: 'Không tìm thấy phòng để chỉnh sửa' });
    }
...
app.post('/api/payment', async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Thanh toán thành công', amount: totalAmount });
  } catch (error) {
...
app.post('/payment', async (req, res) => {
  try {
    ...
    const reservationQuery = 'SELECT ReservationID FROM reservation WHERE RoomID = ?';
    const [reservationResult] = await db.execute(reservationQuery, [roomid]);

    if (reservationResult.length === 0) {
      return res.status(404).json({ message: 'Không tìm thấy đặt phòng với RoomID này.' });
    }

    const reservationID = reservationResult[0].ReservationID;

    // Lưu thông tin thanh toán vào bảng payment
    const paymentQuery = 'INSERT INTO payment (ReservationID, Amount, PaymentMethod) VALUES (?, ?, ?)';
    ...
    const updateRoomQuery = 'UPDATE room SET Status = "Available" WHERE RoomID = ?';
    await db.execute(updateRoomQuery, [roomid]);

    // Trả về thông báo thành công
    res.status(200).json({ message: 'Thanh toán thành công và phòng đã được cập nhật thành trống.' });
  } catch (err) {
...
app.post('/reservation', async (req, res) => {
  const { name, phone, customerid, roomid, checkindate, checkoutdate } = req.body;
  try {
